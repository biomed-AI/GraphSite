...
class Adam12(SchedulerBase):
    ...
    def schedule(self, net, epoch, epochs, **kwargs):
        ...
        if self._cur_optimizer is None:
            # self._cur_optimizer = optim.SGD(net.parameters(), lr=lr, weight_decay=0.00001)
            self._cur_optimizer = optim.Adam(net.parameters(), lr=lr, weight_decay=0.00001, eps=1e-5)#, weight_decay=0.0005
